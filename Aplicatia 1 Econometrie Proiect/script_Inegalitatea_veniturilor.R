...
# Testarea principalelor ipoteze 
...
bptest(model) # nu avem hetero => 0.66 > 0.05
white_test(model) # nu avem hetero => 0.662 > 0.05 
ArchTest(df$uhat) # nu avem hetero conditionata

# Autocorelarea presupune ca Fiecare coloană din grafic reprezintă laguri pentru ca lagurile depășesc intervalul punctat, astfel putem deduce că reziduurile sunt autocorelate. 
# În grafic, se poate observa că avem un singur lag ce depășește intervalul punctat.
...
# Tari ECE - Tari din Europa Centrala si de Est
# 1 pentru tarile care sunt in ece
# 0 pentru cele care nu sunt
...
white_test(model_dummy) # p-value 0.553069 > 0.1 -> sunt homo
